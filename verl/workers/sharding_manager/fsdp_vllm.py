...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(self,
        ...
        self.torch_random_states = torch.cuda.get_rng_state() if not is_npu_available else torch.npu.get_rng_state()
        # get a random rng states
        if self.device_mesh is not None:
            gen_dp_rank = self.device_mesh['dp'].get_local_rank()
            torch.cuda.manual_seed(gen_dp_rank + 1000) if not is_npu_available else \
                torch.npu.manual_seed(gen_dp_rank + 1000)# make sure all tp ranks have the same random states
            self.gen_random_states = torch.cuda.get_rng_state() if not is_npu_available else torch.npu.get_rng_state()
            torch.cuda.set_rng_state(self.torch_random_states) if not is_npu_available else \
    ...
    def __enter__(self):
        ...
        del params
        torch.cuda.empty_cache() if not is_npu_available else torch.npu.empty_cache()
        ...
        if self.device_mesh is not None:
            self.torch_random_states = torch.cuda.get_rng_state() if not is_npu_available else torch.npu.get_rng_state()
            torch.cuda.set_rng_state(self.gen_random_states) if not is_npu_available else \
