...
class FSDPSFTTrainer(object):
    ...
    def _build_model_optimizer(self):
        ...
        with init_context():
            self.model: PreTrainedModel = AutoModelForCausalLM.from_pretrained(local_model_path,
                                                                               ...
                                                                               if not is_npu_available else 'eager',
        ...
        self.fsdp_model = FSDP(module=self.model,
                               ...
                               if not is_npu_available else torch.npu.current_device(),
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context:
            with torch.autocast(device_type='cuda' if not is_npu_available else 'npu', dtype=torch.bfloat16):
                if not use_sp:
...
def main(config):
    ...
    device_mesh = init_device_mesh(device_type='cuda' if not is_npu_available else 'npu', mesh_shape=(world_size,), mesh_dim_names=('fsdp',))
    dp_size = world_size // config.ulysses_sequence_parallel_size
    ulysses_device_mesh = init_device_mesh(device_type='cuda' if not is_npu_available else 'npu',
