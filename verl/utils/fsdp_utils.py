...
def init_fn(x: torch.nn.Module):
    if not torch.distributed.get_rank() == 0:
        x = x.to_empty(device=torch.cuda.current_device() if is_npu_available else torch.npu.current_device(),
                       recurse=False)
        torch.cuda.empty_cache() if not is_npu_available \
            else torch.npu.empty_cache() if not is_npu_available else torch.npu.empty_cache()
    return x
...
def parallel_load_safetensors(filepath):
    ...
    shard_states = {}
    device = torch.cuda.current_device() if is_npu_available else torch.npu.current_device()
...
def parallel_init_module_fn(module: torch.nn.Module, shard_states: Dict[str, torch.nn.Parameter]):
    ...
    def create_and_sync_state(param_name, state, is_param):
        ...
        device = torch.cuda.current_device() if is_npu_available else torch.npu.current_device() \
            if is_npu_available else torch.npu.current_device()
        if is_param:
